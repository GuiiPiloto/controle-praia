    ...
    <div aria-hidden className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
      ...
      {particles.map((p, i) => (
        <div
          key={i}
          className="particle"
          style={{ left: p.left, top: p.top, width: p.size + "px", height: p.size + "px", animationDelay: p.delay }}
        />
      ))}
    </div>
  );
      ...
      <div className="bg-zinc-800/60 border border-zinc-700/50 rounded-2xl p-4 sm:p-5 shadow-lg">
        ...
        <div className="h-[180px] sm:h-[200px]">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={chartData.tipoData} cx="50%" cy="50%" innerRadius={50} outerRadius={75} dataKey="value" animationBegin={0} animationDuration={1000} stroke="none">
                {chartData.tipoData.map((entry, i) => (<Cell key={i} fill={entry.color} />))}
              </Pie>
              <Tooltip contentStyle={{ background: "#18181b", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "12px", fontSize: "13px" }} itemStyle={{ color: "#fff" }} />
            </PieChart>
          </ResponsiveContainer>
        </div>
...
function buildCloudinaryErrorMessage(json, status) {
  ...
  if (normalized.includes("unknown api key")) {
    return [
      base,
      "Verifique se CLOUD_NAME e upload_preset pertencem a mesma conta do Cloudinary e se o preset e unsigned.",
      `CLOUD_NAME atual: ${CLOUD_NAME}`,
      `upload_preset atual: ${CLOUD_UPLOAD_PRESET}`,
    ].join(" | ");
  }
...
async function uploadFoto(file) {
  try {
    ...
    console.log("[Cloudinary] RAW RESPONSE:", rawBody);

    let json = {};
    try {
      json = rawBody ? JSON.parse(rawBody) : {};
    } catch (parseErr) {
      console.error("[Cloudinary] Erro ao parsear JSON da resposta:", parseErr);
    }

    console.log("[Cloudinary] RESPOSTA CLOUDINARY:", json);

    if (!response.ok) {
      throw new Error(buildCloudinaryErrorMessage(json, response.status));
    }

    if (!json?.secure_url) {
      throw new Error("Upload concluido sem secure_url na resposta do Cloudinary");
    }

    return json.secure_url;
  } catch (err) {
    console.error("[Cloudinary] ERRO CLOUDINARY:", err);
    throw err;
  }
}
...
export default function App() {
    ...
    const unsub = onAuthStateChanged(
      ...
      (u) => {
        ...
        if (u) {
          ...
          console.log("[Auth] Nenhum usuário autenticado no momento.");
        }
      },
      (err) => {
        console.error("[Auth] Erro ao observar autenticação:", err);
      }
    );
    return () => unsub();
  }, []);
  ...
  useEffect(() => {
    const unsub = onSnapshot(
      ...
      (snapshot) => {
        console.log("[Firestore] Snapshot recebido com", snapshot.size, "documentos.");
      ...
      (err) => {
        console.error("[Firestore] Erro ao conectar/ler coleção pessoas:", err);
        toast.error("Erro ao conectar com o Firestore");
        setLoading(false);
      }
    );
    return () => unsub();
    ...
    try {
      ...
      if (file) {
        try {
          ...
          console.error("[AddPessoa] Falha no upload da foto:", err);
          toast.error("Erro ao fazer upload da foto: " + getErrorMessage(err));
        }
      }
      await addDoc(collection(db, "pessoas"), {
        nome: nomeLimpo, foto: fotoUrl, tipo: "avista", p1: false, p2: false, createdAt: serverTimestamp(),
      });
      ...
      console.error("[AddPessoa] Erro ao adicionar pessoa:", err);
      toast.error("Erro ao adicionar pessoa: " + getErrorMessage(err));
    } finally {
      setAdding(false);
    }
  };
  ...
  return (
    <div className="relative min-h-screen bg-zinc-900 flex flex-col overflow-x-hidden">
      <Toaster position="top-right" toastOptions={{
        style: { background: "#18181b", color: "#fff", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "16px", padding: "14px 18px", fontSize: "14px", backdropFilter: "blur(12px)", boxShadow: "0 8px 32px rgba(0,0,0,0.4)" },
        success: { iconTheme: { primary: "#34d399", secondary: "#fff" } },
        error: { iconTheme: { primary: "#ef4444", secondary: "#fff" } }, duration: 3000,
      }} />
      <BeachBackground />
      <header className="relative z-20 w-full py-4 sm:py-5 md:py-6 lg:py-8 bg-zinc-900/90 border-b border-white/10 shadow-lg sticky top-0">
        <div className="max-w-4xl mx-auto flex items-center justify-between px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-0.5">
            <h1 className="text-responsive-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-white tracking-tight leading-tight pr-2">Controle Financeiro Praia</h1>
            <span className="text-[10px] sm:text-xs text-cyan-400/80 font-medium tracking-widest uppercase">{"☀"} Verão 2026</span>
          </div>
          <button onClick={handleLogin} className="flex-shrink-0 bg-zinc-800 hover:bg-zinc-700 active:bg-zinc-600 text-white px-4 sm:px-5 py-2 sm:py-2.5 rounded-2xl shadow transition-all duration-200 font-semibold border border-zinc-700 hover:border-cyan-500/50 text-responsive-sm sm:text-sm min-h-touch flex items-center">
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-start py-4 sm:py-6 md:py-8 lg:py-10 px-3 sm:px-4 md:px-6">
        <div className="w-full max-w-4xl mx-auto">
          <CountdownTimer />
          {isAdmin && (
            <form onSubmit={addPessoa} className="flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-4 mb-6 md:mb-8 items-stretch sm:items-center bg-zinc-800/90 p-4 sm:p-5 md:p-6 rounded-2xl shadow-lg border border-zinc-700/50">
              <div className="flex-1 flex items-center gap-3 min-w-0">
                <input value={nome} onChange={(e) => setNome(e.target.value)} placeholder="Adicionar pessoa..." className="flex-1 bg-zinc-900 text-white px-4 py-3 sm:py-3.5 rounded-2xl outline-none border border-zinc-700 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/30 text-responsive-base sm:text-lg shadow min-h-touch transition-all duration-200" />
              </div>
              {isAdmin && (
                <div className="flex items-center gap-2">
                  {fotoPreview ? (
                    <div className="relative flex-shrink-0">
                      <motion.div initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-[48px] h-[48px] rounded-full overflow-hidden ring-2 ring-cyan-400/60 ring-offset-2 ring-offset-zinc-800">
                        <img src={fotoPreview} alt="Preview" className="w-full h-full object-cover" />
                      </motion.div>
                      <button type="button" onClick={clearFotoPreview} disabled={adding} className="absolute -top-1 -right-1 bg-red-600 hover:bg-red-700 text-white w-[20px] h-[20px] rounded-full flex items-center justify-center text-xs font-bold shadow-lg transition-all">{"×"}</button>
                    </div>
                  ) : (
                    <label className="flex-shrink-0 bg-blue-700 hover:bg-blue-800 active:bg-blue-900 text-white px-4 sm:px-5 py-3 sm:py-3.5 rounded-2xl font-semibold shadow cursor-pointer transition-all duration-200 border border-blue-800 min-h-touch flex items-center justify-center text-sm sm:text-base gap-1.5">
                      {"📸"} Foto
                      <input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={handleFotoSelect} ref={fileInputRef} disabled={adding} />
                    </label>
                  )}
                </div>
              )}
              <button type="submit" disabled={adding} className={"flex-shrink-0 bg-gradient-to-br from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 active:from-cyan-600 active:to-blue-700 text-white px-6 sm:px-8 py-3 sm:py-3.5 rounded-2xl font-bold shadow-lg shadow-cyan-500/20 transition-all duration-200 min-h-touch min-w-touch flex items-center justify-center text-responsive-lg sm:text-xl " + (adding ? "opacity-60 cursor-not-allowed" : "")}>
                {adding ? "..." : "+"}
              </button>
            </form>
          )}
          {isAdmin && (
            <>
              <motion.div className="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 md:gap-6 mb-6 md:mb-10" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }}>
                {loading ? (<><SkeletonStatusCard /><SkeletonStatusCard /><SkeletonStatusCard /></>) : (<>
...
function StatusCard({ title, value, icon, gradient }) {
  return (
    <motion.div className={"relative rounded-2xl p-4 sm:p-5 md:p-6 bg-gradient-to-br " + gradient + " text-white flex flex-col items-center justify-center min-h-[100px] sm:min-h-[120px] shadow-lg overflow-hidden group cursor-default"} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} whileHover={{ y: -3, scale: 1.02 }} layout>
...
function PessoaCard({ pessoa, db, user, onDelete }) {
  const id = pessoa.id;
  ...
  const handleToggleTipo = async () => {
    if (!id) {
      toast.error("Documento inválido para alterar tipo.");
      return;
    }
    try {
      ...
      console.error("[UpdatePessoa] Erro ao alterar tipo:", err);
      toast.error("Erro ao alterar tipo: " + getErrorMessage(err));
    }
  };

  const handleP1 = async () => {
    if (!id) {
      toast.error("Documento inválido para atualizar pagamento.");
      return;
    }
    try {
      ...
      if (novoValor) {
        ...
        if (tipo === "avista") enviarWhats("💸 *" + nome + "* quitou tudo à vista!");
        else enviarWhats("💰 *" + nome + "* pagou a *1ª parcela*!");
      }
    } catch (err) {
      console.error("[UpdatePessoa] Erro ao atualizar pagamento 1:", err);
      toast.error("Erro ao atualizar pagamento: " + getErrorMessage(err));
    }
  };

  const handleP2 = async () => {
    if (!p1) { toast.error("Marque a 1ª parcela primeiro"); return; }
    if (!id) {
      toast.error("Documento inválido para atualizar pagamento.");
      return;
    }
    try {
      ...
      if (novoValor) { toast.success(nome + " - 2ª parcela paga! 🎉"); enviarWhats("💰 *" + nome + "* pagou a *2ª parcela*!"); }
    } catch (err) {
      console.error("[UpdatePessoa] Erro ao atualizar pagamento 2:", err);
      toast.error("Erro ao atualizar pagamento: " + getErrorMessage(err));
    }
  };
  ...
  const salvarNome = async () => {
    ...
    if (!nomeTrim || nomeTrim === nome) { setEditandoNome(false); setNomeEdit(nome); return; }
    if (!id) {
      toast.error("Documento inválido para atualizar nome.");
      return;
    }
    ...
    try {
      ...
      console.error("[UpdatePessoa] Erro ao atualizar nome:", err);
      toast.error("Erro ao atualizar nome: " + getErrorMessage(err));
    } finally {
  ...
  const handleFotoEdit = async (e) => {
    ...
    if (!file) return;
    if (!id) {
      toast.error("Documento inválido para atualizar foto.");
      return;
    }
    ...
    try {
      ...
      console.error("[UpdatePessoa] Erro ao atualizar foto:", err);
      toast.error("Erro ao atualizar foto: " + getErrorMessage(err));
    } finally { setEditandoFoto(false); }
  };

  return (
    <motion.div className="relative bg-zinc-800 rounded-2xl shadow-lg p-4 sm:p-5 md:p-6 flex flex-col gap-4 sm:gap-5 border border-zinc-700 hover:border-cyan-500/30 transition-all duration-300 overflow-hidden group" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 30 }} whileHover={{ scale: 1.005 }} layout>
        ...
        <div className="relative flex-shrink-0">
          ...
          {isAdmin && !editandoFoto && (
            <label className="absolute -bottom-1 -right-1 w-[22px] h-[22px] bg-zinc-700 hover:bg-zinc-600 rounded-full flex items-center justify-center cursor-pointer border border-zinc-600 shadow-lg transition-all hover:scale-110">
              ...
              <input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={handleFotoEdit} disabled={editandoFoto} />
            </label>
          )}
          {editandoFoto && <div className="absolute -bottom-1 -right-1 w-[22px] h-[22px] rounded-full flex items-center justify-center bg-cyan-500/30 border border-cyan-400/50"><span className="text-[8px] text-cyan-300 animate-pulse">⬆</span></div>}
        </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {editandoNome ? (
                <div className="flex items-center gap-2 w-full">
                  <input value={nomeEdit} onChange={(e) => setNomeEdit(e.target.value)} className="flex-1 bg-zinc-900 text-white px-3 py-1.5 rounded-xl outline-none border border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/30 text-responsive-lg sm:text-xl md:text-2xl font-bold min-h-[44px]" autoFocus onKeyDown={(e) => e.key === "Enter" && salvarNome()} />
                  <button onClick={salvarNome} disabled={salvandoNome} className="bg-emerald-700 hover:bg-emerald-600 text-white px-3 py-1.5 rounded-xl font-bold text-xs shadow transition-all disabled:opacity-50">{salvandoNome ? "..." : "Salvar"}</button>
                  <button onClick={() => { setEditandoNome(false); setNomeEdit(nome); }} className="bg-zinc-700 hover:bg-zinc-600 text-white px-3 py-1.5 rounded-xl font-bold text-xs shadow transition-all">Cancelar</button>
                </div>
              ) : (
                <>
                  <span className={"text-responsive-lg sm:text-xl md:text-2xl font-bold text-white truncate " + (isAdmin ? "cursor-pointer hover:text-cyan-300 transition-colors" : "")} onClick={() => { if (isAdmin) { setNomeEdit(nome); setEditandoNome(true); } }} title={isAdmin ? "Clique para editar" : ""}>{nome}</span>
                  {isAdmin && <span className="flex-shrink-0 text-[10px] sm:text-xs bg-cyan-500/20 text-cyan-300 px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-xl font-medium border border-cyan-400/30">ADM</span>}
                </>
              )}
            </div>
          </div>
          <button onClick={handleToggleTipo} className={"flex-shrink-0 px-3 py-1.5 sm:px-4 sm:py-1.5 rounded-2xl text-responsive-xs sm:text-sm font-semibold shadow border transition-all duration-200 active:scale-95 " + (tipo === "avista" ? "bg-emerald-700 text-white border-emerald-600 hover:bg-emerald-600" : "bg-amber-700 text-white border-amber-600 hover:bg-amber-600")}>{tipo === "avista" ? "À vista" : "Parcelado"}</button>
        </div>
      </div>
      <div className="relative z-10 flex flex-col sm:flex-row gap-3 sm:gap-4 items-stretch sm:items-center">
        <div className="flex gap-2 sm:gap-3 min-w-0 sm:min-w-[160px]">
          <button onClick={handleP1} className={"flex-1 px-3 sm:px-4 py-2.5 sm:py-3 rounded-2xl font-bold shadow border transition-all duration-200 text-responsive-sm sm:text-base min-h-touch active:scale-95 " + (p1 ? "bg-emerald-600 text-white border-emerald-700 hover:bg-emerald-700 shadow-lg shadow-emerald-600/20" : "bg-zinc-700 text-white border-zinc-600 hover:bg-zinc-600")}>{tipo === "avista" ? (p1 ? "✓ Pago" : "Pagar") : p1 ? "✓ 1ª" : "Pagar 1ª"}</button>
          {tipo === "parcelado" && <button onClick={handleP2} className={"flex-1 px-3 sm:px-4 py-2.5 sm:py-3 rounded-2xl font-bold shadow border transition-all duration-200 text-responsive-sm sm:text-base min-h-touch active:scale-95 " + (p2 ? "bg-emerald-600 text-white border-emerald-700 hover:bg-emerald-700 shadow-lg shadow-emerald-600/20" : "bg-zinc-700 text-white border-zinc-600 hover:bg-zinc-600")}>{p2 ? "✓ 2ª" : "Pagar 2ª"}</button>}
        </div>
        <div className="flex flex-col justify-center flex-1 min-w-0">
          <div className="h-3 sm:h-3.5 md:h-4 bg-zinc-700 rounded-xl overflow-hidden shadow-inner">
            <motion.div className="h-full bg-gradient-to-r from-cyan-400 via-green-400 to-emerald-500 rounded-xl relative" style={{ width: progresso + "%" }} initial={{ width: 0 }} animate={{ width: progresso + "%" }} transition={{ duration: 0.8, ease: "easeOut" }}>
      ...
      {isAdmin && (
        <div className="relative z-10 flex items-center justify-end flex-wrap gap-3 pt-1 border-t border-zinc-700/50">
          <button onClick={onDelete} className="bg-red-700 hover:bg-red-800 active:bg-red-900 text-white px-4 sm:px-5 py-2 sm:py-2.5 rounded-2xl font-bold shadow border border-red-800 transition-all duration-200 min-h-touch flex items-center text-responsive-xs sm:text-sm gap-1.5">{"🗑"} Excluir</button>
        </div>
      )}
    </motion.div>
  );
